...
import { ListFilesCommandContribution } from './jsonschema-form-contribution';
...
describe('ListFilesCommandContribution', () => {
  function setup(selectionUri:string, result:string[], state:any) {
      const container = new Container();
      container.bind(ListFilesCommandContribution).toSelf().inSingletonScope();
      ...
      container.bind(ListFilesService).toConstantValue({listFiles: () => Promise.resolve(result)});
  ...
  it('should pass to widget manager the same array provided by ListFilesService', async () => {
      ...
      const impl = setup('ciao', mockedList, state).get(ListFilesCommandContribution);
